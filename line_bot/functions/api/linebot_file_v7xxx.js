...
async function handleFileMessage(event) {
    ...
    try {
        ...
        */
        ...
        /*
        // 6. 美化版回應訊息 下載按鈕有bug 不用了
        // 使用 Flex Message 卡片回覆
        const fileInfoCard = createFileInfoCard({
            fileName: originalFileName || '未命名檔案',
            fileType: messageType,
            fileSize: buffer.length,
            downloadUrl: encodeURI(downloadURL),
            uploadedAt: new Date().toISOString(),
            duration: event.message.duration > 0 && (messageType === 'audio' || messageType === 'video') ? event.message.duration : null
        });
        await client.replyMessage({
            replyToken: event.replyToken,
            messages: [fileInfoCard]
        });  */
...
// 美化版回覆上傳檔案資訊 下載按鈕有bug不好用放著
/**
 * 生成檔案資訊卡片（Flex Message）
 * @param {Object} fileInfo - 檔案資訊
 * @param {string} fileInfo.fileName - 檔名
 * @param {string} fileInfo.fileType - 檔案類型 (image/video/audio/file)
 * @param {number} fileInfo.fileSize - 檔案大小（位元組）
 * @param {string} fileInfo.downloadUrl - 下載網址
 * @param {string} fileInfo.uploadedAt - 上傳時間（ISO 字串）
 * @param {number|null} fileInfo.duration - 時長（毫秒，僅音訊/影片）
 */
function createFileInfoCard(fileInfo) {
    const fileSizeKB = Math.round(fileInfo.fileSize / 1024);
    const fileSizeMB = (fileInfo.fileSize / (1024 * 1024)).toFixed(2);
    ...
    if (fileInfo.duration && fileInfo.duration > 0) {
        const durationSeconds = Math.floor(fileInfo.duration / 1000);
    ...
    bodyContents.push({
        type: "text",
        text: `📅 上傳時間：${new Date(fileInfo.uploadedAt).toLocaleString('zh-TW')}`,
        size: "sm",
        color: "#666666"
    });
    ...
    return {
        type: "flex",
        altText: `檔案上傳成功：${fileInfo.fileName}`,
        contents: {
            type: "bubble",
            size: "kilo",
            header: {
                ...
                contents: [
                    {
                        type: "text",
                        text: `${fileTypeEmoji[fileInfo.fileType] || '📄'} 檔案上傳成功`,
                        weight: "bold",
                        size: "lg",
                        color: "#ffffff"
                    }
